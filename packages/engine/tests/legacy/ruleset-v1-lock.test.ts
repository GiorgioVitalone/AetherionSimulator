...
d('ruleset-v1 lock', () => {
  ...
  it('archived gameplay rules remain deterministic under the current corpus without rewriting the historical pool pin', async () => {
    const manifest = readManifest();
    const { runSim } = (await import(runnerPath)) as {
      runSim: (c: unknown) => { runHash: string };
    };
    const config = { ...TINY_BASE, rulesProfile: 'legacy-v1', ...manifest.rules };
    const result = runSim(config);
    expect(runSim(config).runHash).toBe(result.runHash);
    expect(result.runHash).not.toBe(manifest.gameplayPin.runHash);
  }, 30000);

  it('the explicit flags-off diagnostic is deterministic without impersonating the archived pool pin', async () => {
    const manifest = readManifest();
    const { runSim } = (await import(runnerPath)) as {
      runSim: (c: unknown) => { runHash: string };
    };
    const config = { ...TINY_BASE, rulesProfile: 'custom-diagnostic' };
    const result = runSim(config);
    expect(runSim(config).runHash).toBe(result.runHash);
    expect(result.runHash).not.toBe(manifest.legacyPin.runHash);
  }, 30000);
